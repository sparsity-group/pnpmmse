...
class LPN(nn.Module):
    def __init__(
        ...
        self.lin = nn.ModuleList(
            [
                nn.Conv2d(
                    in_dim, hidden, 3, bias=True, stride=1, padding=1, dilation=1
                ),  # 28
                nn.Conv2d(
                    hidden, hidden, 3, bias=True, stride=2, padding=1, dilation=1
                ),  # 14
                nn.Conv2d(
                    hidden, hidden, 3, bias=True, stride=1, padding=1, dilation=1
                ),  # 14
                nn.Conv2d(
                    hidden, hidden, 3, bias=True, stride=2, padding=1, dilation=1
                ),  # 7
                nn.Linear(hidden * 7 * 7, 64, bias=True),
