...
def calculate_moreau_envelope_learned(xi, model, prior_func, prox_func, lambda_ = 1):
    x_star = prox_func(xi, model)
    prior_val = prior_func(x_star, model)
    quadratic_term = .5 *np.square(x_star - xi)
    return prior_val + quadratic_term